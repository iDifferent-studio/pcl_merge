...
try:
    ...
    from sensor_msgs_py.numpy_compat import (structured_to_unstructured, unstructured_to_structured)
...
def do_transform_cloud(cloud: PointCloud2, transform: Union[Transform, TransformStamped]) -> PointCloud2:
    """
    ...
    assert required_fields <= present_fields, 'Point cloud needs the fields x, y, and z for the transformation'
    ...
    transformed_xyz = transform_points(points[:,:3], transform)
    ...
    if required_fields != present_fields:
        ...
        points = np.column_stack((transformed_xyz,points[:,3:]))
    else:
